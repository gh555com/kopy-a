...
class TransparentPopup(QWidget):
    ...
    def setup_ui(self):
        ...
        self.top_content.setMaximumHeight(162); self.top_content.setFixedWidth(202)
        ...
        layout.addWidget(self.top_content, 0, Qt.AlignHCenter); layout.addStretch()
    ...
    def mousePressEvent(self, event):
        child_rects = [self.overlay_scrollbar.geometry()] if self.overlay_scrollbar.isVisible() else []
        child_rects.extend([self.top_content.geometry(), self.bottom_message_label.geometry()])
        is_on_child = any(rect.contains(event.pos()) for rect in child_rects)
        if is_on_child: super().mousePressEvent(event)
        elif event.button() == Qt.LeftButton and not self.is_sticky: self.slide_out()
